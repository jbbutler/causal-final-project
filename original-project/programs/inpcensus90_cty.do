# delimit ;
capture log close;
log using "$logs/inpcensus90_cty.log", replace;
...
This program inputs data from the 1990 Census to be used to examine whether
the introduction of Sesame Street had an impact on educational attainment. This
...
infix 
  ...
  byte    uhrswork   94-95   
  long    incwage    96-101  
  long    incwelfr   102-106 
  int     poverty    107-109 
  int     migplac5   110-112 
  using "$raw/census1990.dat";
...
gen raceeth = "whitenh" if whitenh == 1;
     replace raceeth = "blacknh" if blacknh == 1;
	 replace raceeth = "othernh" if othernh == 1;
	 replace raceeth = "hispanic" if hispanic == 1;
	 
gen move = stbirth ~= stfips;
tab move;
qui sum move;
global stay1990 = 1-r(mean);
qui sum move [aw=perwt];
global stay1990_wt = 1-r(mean);
keep if move == 0;
...
merge oldcode using "$raw/ludwig_miller_fipsrecode.dta";
tab _merge;
keep if _merge == 3;
drop _merge;
...
save "$temp/educearn90_cty.dta", replace;

sort stbirth ctyfips yearage6;
collapse (mean) hsdrop hsgrad anycol (count) numobs90=hsdrop, 
     by(stbirth ctyfips yearage6);
sort stbirth ctyfips yearage6;
save "$temp/aggeduc90_cty.dta", replace;
