...
logistic_model <- glm(split ~ dists_10 + is_uhf + kw_100 + hgt_100 + dists_10*is_uhf, family=binomial(logit), weights = cov_df$total_hh, data=cov_df_logistic)
...
SSstation_cty_final
saveRDS(SSstation_cty_final, file=paste0(path, 'SScoverage.Rda'))
```
...
st_fips <- sprintf("%02d",st_fips)
cty_fips <- sprintf("%03d",cty_fips)
fips_tot <- paste0(st_fips, cty_fips)

# discretize the predicted coverage variable
plt_df <- data.frame(fips=fips_tot, pred_coverage=SSstation_cty_final$max_pred_cov)
discrete <- rep(0, nrow(plt_df))
discrete[plt_df$pred_coverage < 50] = 1
discrete[plt_df$pred_coverage >= 50 & plt_df$pred_coverage < 60] = 2
discrete[plt_df$pred_coverage >= 60 & plt_df$pred_coverage < 70] = 3
discrete[plt_df$pred_coverage >= 70 & plt_df$pred_coverage < 80] = 4
discrete[plt_df$pred_coverage >= 80] = 5
plt_df <- plt_df %>% mutate(pred_cov_discrete = as.factor(discrete))

# plot
plot_usmap(data=plt_df, values='pred_cov_discrete', size = 0.1) + scale_fill_brewer(name = 'Coverage', labels =c('< 50%', '50-60%', '60-70%', '70-80%', '>80%', 'No Data'))
```
...
```{r}
# discretize the predicted coverage variable
plt_df <- data.frame(fips=fips_tot, pred_coverage=SSstation_logistic_final$max_pred_cov*100)
discrete <- rep(0, nrow(plt_df))
discrete[plt_df$pred_coverage < 50] = 1
discrete[plt_df$pred_coverage >= 50 & plt_df$pred_coverage < 60] = 2
discrete[plt_df$pred_coverage >= 60 & plt_df$pred_coverage < 70] = 3
