...
logistic_model <- glm(split ~ dists_10 + is_uhf + kw_100 + hgt_100 + dists_10*is_uhf, family=binomial(logit), data=cov_df_logistic)
...
```{r}
# get coverage predictions
SScov_preds <- predict(logistic_model, SSstation_cty_pairs, type = 'response')
SSstation_cty_covs <- SSstation_cty_pairs %>% mutate(pred_coverage = SScov_preds)
...
sum(SSstation_cty_final$pop70*(SSstation_cty_final$max_pred_cov))/sum(SSstation_cty_final$pop70)
```
