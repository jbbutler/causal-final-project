...
```{r}
logistic_mod1 <- glm(only_hsgrad ~ hsspend_at_four + fs_available + blacknh + female + 
                       othernh + hispanic + preschool1969:pred_cov, 
                     family = 'gaussian', data = reg_dat)
...
```{r}
logistic_mod1 <- glm(any_college ~ hsspend_at_four + fs_available + blacknh + female + 
                       othernh + hispanic + preschool1969:pred_cov, 
                     family = 'gaussian', data = reg_dat)
...
```{r}
# making Table 6, but multiplying by 30 percentage points and exponentiating to get the
# factorial increase in odds of being at grade-level
...
kable(tab6_df, caption = 'Odds ratio of outcomes when SS coverage is increased by 30 percentage points') %>% kable_classic()
```
Results by-and-large seem to match what they have!