capture log close
log using "$logs/table4.log", replace
...
*Some data prep
do "$bs_dofiles/census80_toestimate.do"
	
*First we must get and store the point estimates
cd "$final"
use census80, clear
...
forval i = 1/`num' {
	...
	*Estimate the FE model
	...
	matrix define census4a[1,`i'] = _b[preschlcov]
	matrix define census4a[3,`i'] = e(N) //preserve sample size
	qui sum gradeage if e(sample)
	matrix define census4a[4,`i'] = r(mean) //preserve dependent mean
	...
	matrix define census4b[1,`i'] = _b[covst6768]
	matrix define census4b[3,`i'] = _b[covst69]
	matrix define census4b[5,`i'] = _b[covst7072]
	matrix define census4b[7,`i'] = _b[covst7374]
	matrix define census4b[9,`i'] = e(N)

}

**************************************
****BOOTSTRAP PROGRAM FOR TABLE 4 - S.E. CLUSTERED AT DMA
**************************************
*We will cluster the Bootstrap at the DMA level
cap program drop bootstrapcensus4_dma
program define bootstrapcensus4_dma, rclass
	cd "$final"
	*First Stage
...
simulate tot_group1 = r(tot_group1) tot_group2 = r(tot_group2) ///
		 ...
		 ///
		 , reps(400) saving(census_bootstrap_4dma, replace) seed(3865788): bootstrapcensus4_dma

/**/
/**/

********************
*******Table 4 (S.E. clustered at DMA level)
cd "$final"
use census_bootstrap_4dma, clear
cd "$tables"
forval i = 1/6 {
	qui sum tot_group`i'
	matrix define census4a[2,`i'] = r(sd)
	forval j = 2(2)8 {
		qui sum yr_group`i'_`j'
		matrix define census4b[`j',`i'] = r(sd)
	}
}

*Grade for Age Dummy treatment by Gender and Ethnicity
cap file close census
file open census using census_table4dma.txt, write replace
...
forval j = 1(2)7 {
	...
	forval i = 1/6 {
		file write census _tab %7.3f (census4b[`j',`i'])
	}
	file write census _n
	forval i = 1/6 {
		file write census _tab "(" %5.3f (census4b[`k',`i']) ")"
	}
	file write census _n
}
file write census _n 
...
forval i = 1/6 {
	file write census _tab %9.0gc (census4a[3,`i'])
}
file close census
